...
//        logger.info(oAuth2User.getAttributes().toString());
        ...
        if (!authenticationService.isRegistered(user.getEmail())) {
            UserBasic userSave = authenticationService.signupGitHub(UserMapper.toRegisterUserDto(user), user.getGithubId());
            CookiesUtils.addUserCookie(UserMapper.toDto(userSave));
        } else {
